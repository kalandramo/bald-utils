...
import (
	...
	"github.com/kalandramo/bald-utils/captcha"
)

// ExampleNewCaptcha 使用 Options 模式创建验证码（推荐）
...
// Example_captchaDigitCaptcha 数字验证码示例
func Example_captchaDigitCaptcha() {
	rdb := redis.NewClient(&redis.Options{
		Addr: "localhost:6379",
	})
...
// Example_captchaMathCaptcha 算术验证码示例
func Example_captchaMathCaptcha() {
	rdb := redis.NewClient(&redis.Options{
		Addr: "localhost:6379",
	})
...
// Example_captchaChineseCaptcha 中文验证码示例
func Example_captchaChineseCaptcha() {
	rdb := redis.NewClient(&redis.Options{
		Addr: "localhost:6379",
	})
